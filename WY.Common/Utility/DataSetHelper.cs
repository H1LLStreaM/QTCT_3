    ...
    public class DataSetHelper
    {
        ...
        string rowFilter, string groupBy)
        {
            ...
            foreach (DataRow sourceRow in rows)
            {
                ...
                foreach (FieldInfo field in GroupByFieldInfo)
                {
                    if (string.IsNullOrEmpty(field.Aggregate))
                    {
                        destRow[field.FieldAlias] = sourceRow[field.FieldName];
                    }
                    else
        ...
        public DataTable CreateTable(string tableName, string fieldList)
        {
            ...
            {
                FieldsParts = Field.Trim().Split(" ".ToCharArray(), 3); // allow for spaces in the e­xpression 
                ...
                else if (FieldsParts.Length == 3) // add fieldname, datatype, and e­xpression 
                {
